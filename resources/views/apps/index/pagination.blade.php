        ...
        <div class="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
            <div class="hidden sm:block">
                <p class="text-sm leading-5 text-gray-700">
                    <span>{{ trans('frontend.apps.pagination.showing') }}</span>
                    <span class="font-medium">{{ $paginator->toArray()['from'] }}</span>
                    <span>{{ trans('frontend.apps.pagination.to') }}</span>
                    <span class="font-medium">{{ $paginator->toArray()['to'] }}</span>
                    <span>{{ trans('frontend.apps.pagination.of') }}</span>
                    <span class="font-medium">{{ $paginator->toArray()['total'] }}</span>
                    <span>{{ trans('frontend.apps.pagination.results') }}</span>
                </p>
            </div>
            <div class="flex-1 flex justify-between sm:justify-end">
                <div>
                    @if (!$paginator->onFirstPage())
                        ...
                        >
                            {{ trans('frontend.apps.pagination.previous') }}
                        </button>
                    @endif
                </div>
                <div>
                    @if ($paginator->hasMorePages())
                        ...
                        >
                            {{ trans('frontend.apps.pagination.next') }}
                        </button>
                    @endif
                </div>
            </div>
