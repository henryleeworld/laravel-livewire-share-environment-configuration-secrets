<div class="bg-white rounded-lg overflow-hidden shadow-xl mx-auto">
    <form wire:submit.prevent="update" spellcheck="false">
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div class="flex items-center justify-between flex-wrap sm:flex-no-wrap">
                <div>
                    <h3 class="text-lg leading-6 font-medium text-gray-900">
                        {{ trans('frontend.users.edit.content.edit') }} {{ $user->full_name }}
                    </h3>
                </div>
            ...
            <x-inputs.text label="{{ trans('frontend.users.edit.content.first_name') }}" name="firstName" :key="$user->id" context="edit" class="mt-6" />

            <x-inputs.text label="{{ trans('frontend.users.edit.content.last_name') }}" name="lastName" :key="$user->id" context="edit"
                           class="mt-6 sm:border-t sm:border-gray-200 sm:pt-5"
            />

            <x-inputs.text label="{{ trans('frontend.users.edit.content.email_address') }}" name="email" :key="$user->id" context="edit" type="email"
                           maxWidth="max-w-lg" class="mt-6 sm:border-t sm:border-gray-200 sm:pt-5"
            />

            <x-inputs.select :disabled="auth()->user()->cannot('updateRole', $user)" label="{{ trans('frontend.users.edit.content.role') }}" name="role"
                ...
                <div class="mt-1 sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start">
                    ...
                    <div class="mt-1 sm:mt-0 sm:col-span-2">
                        <div class="text-xs max-w-xs text-gray-700">
                            @if ($role == 'owner')
                                <p>
                                    {{ trans('frontend.users.edit.content.owners_have_no_restrictions') }}
                                </p>
                            @elseif ($role == 'admin')
                                <p>
                                    {{ trans('frontend.users.edit.content.admins_can_manage_every_app') }}
                                </p>
                            @else
                                <p>
                                    {{ trans('frontend.users.edit.content.users_cannot_access_any_apps') }}
                                </p>
                            @endif
                        </div>
                    </div>
        ...
        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse sm:justify-between">
            <span class="flex w-full rounded-md shadow-sm sm:ml-3 sm:w-auto">
                ...
                >
                    {{ trans('frontend.users.edit.content.save') }}
            ...
            <div class="sm:flex sm:flex-row-reverse">
                ...
                <span class="mt-3 flex w-full rounded-md shadow-sm sm:mt-0 sm:w-auto">
                    ...
                    >
                        ...
                        {{ trans('frontend.users.edit.content.back') }}
                    </button>
                </span>
            </div>
        </div>
