        ...
        <form wire:submit.prevent="request" spellcheck="false" class="space-y-6">
            <div>
                <div class="rounded-md">
                    <input wire:model.defer="email" value="{{ $email }}" type="email" placeholder="{{ trans('frontend.auth.content.your_email_address') }}"
            ...
            <div>
                <span class="block w-full rounded-md shadow-sm">
                    ...
                    >
                        {{ trans('frontend.auth.content.continue') }}
                    </button>
                </span>
            </div>
        </form>
    @else
        <form wire:submit.prevent="confirm" spellcheck="false">
                ...
                <div class="mb-6 rounded-md bg-green-500 p-4 flex">
                    ...
                    <div class="ml-3">
                        <p class="text-sm leading-5 font-medium text-white">
                            {{ trans('frontend.auth.content.a_fresh_code_has_been_sent') }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="text-gray-100">
                {{ trans('frontend.auth.content.shot_an_email_into_your_inbox') }}
            </div>

            <div class="mt-3">
                <div class="rounded-md">
                    <input wire:model.defer="token" placeholder="{{ trans('frontend.auth.content.the_code_we_emailed_you') }}" required autofocus
            ...
            <div class="mt-6 flex items-center justify-end">
                <div class="text-sm leading-5">
                    ...
                    >
                        {{ trans('frontend.auth.content.resend_code') }}
                    </a>
                </div>
            </div>
